...
from app.models.enums import ApplicabilityStatus, AuditStatus, Role, ScopeSource
...
def suggestion_payload(*clause_ids: str, saq: str = "D", ambiguous: bool = False) -> dict[str, Any]:
    return {
        ...
        "requirements": [
            {"control_id": c, "rationale": f"Applies because of clause {c}."} for c in clause_ids
        ],
    }
...
class TestScopeSuggestionHappyPath:
    def test_proposes_requirements_none_confirmed(
        self, api_client: TestClient, make_user: Any, make_audit: Any, corpus: None
    ) -> None:
        ...
        response = api_client.post(f"/api/audits/{audit.id}/scope-suggestion")
    ...
    def test_suggestions_are_marked_ai_suggested(
        ...
        body = api_client.post(f"/api/audits/{audit.id}/scope-suggestion").json()

        assert body["proposed_requirements"][0]["source"] == "ai_suggested"
        assert body["proposed_requirements"][0]["rationale"]

    def test_success_advances_the_audit_to_scoping(
        self,
        ...
        api_client.post(f"/api/audits/{audit.id}/scope-suggestion")

        db.refresh(audit)
        assert audit.status == AuditStatus.scoping
    ...
    def test_only_profile_fields_reach_the_llm(
        ...
        set_llm_client(fake)
        auditor = make_user(Role.auditor, password=PASSWORD)
        audit = make_audit(auditor, client_name="Northwind Retail")
        login(api_client, auditor)
    ...
    def test_hallucinated_clause_ids_are_dropped(
        ...
        ScopedControl pointing at nothing would break every downstream step."""
        ...
        assert [r["control_id"] for r in body["proposed_requirements"]] == ["1.2.1"]
...
class TestScopeSuggestionDegradation:
    ...
    def test_degraded_run_leaves_the_audit_in_intake(
        self,
        api_client: TestClient,
        db: DBSession,
        make_user: Any,
        ...
        """01_REQUIREMENTS.md: "audit remains in intake"."""
        ...
        db.refresh(audit)
        assert audit.status == AuditStatus.intake
    ...
    def test_manual_scoping_still_works_after_a_degraded_run(
        ...
        added = api_client.post(
            f"/api/audits/{audit.id}/scoped-requirements",
            json={"control_id": "1.2.1", "rationale": "Scoped manually."},
        )
    ...
    def test_a_fully_manual_audit_reaches_in_progress(
        self,
        api_client: TestClient,
        db: DBSession,
        make_user: Any,
        ...
        An audit scoped entirely by hand — the documented path when the LLM
        ...
        *successful* AI suggestion advanced it to `scoping`. Since `intake` has
        ...
        api_client.post(f"/api/audits/{audit.id}/scope-suggestion")
        db.refresh(audit)
        assert audit.status == AuditStatus.intake, "degraded run stays in intake"

        added = api_client.post(
            f"/api/audits/{audit.id}/scoped-requirements",
            json={"control_id": "1.2.1"},
        ).json()
        db.refresh(audit)
        assert audit.status == AuditStatus.scoping, "adding scope begins scoping"

        api_client.patch(f"/api/scoped-requirements/{added['id']}", json={"confirmed": True})
        db.refresh(audit)
        assert audit.status == AuditStatus.in_progress, (
            "confirming scope must reach in_progress, or the audit can never finalize"
        )

    def test_a_manually_scoped_audit_can_be_finalized(
        self,
        api_client: TestClient,
        db: DBSession,
        make_user: Any,
        ...
        assert response.status_code == 200, response.text
        assert response.json()["audit_status"] == "finalized"
...
class TestScopeSuggestionPreconditions:
    def test_missing_profile_fields_returns_409(
        ...
        auditor = make_user(Role.auditor, password=PASSWORD)
        audit = make_audit(auditor)
        audit.merchant_level = None  # merchant without a level
        db.flush()
        login(api_client, auditor)
    ...
    def test_rate_limited_after_the_hourly_cap(
        ...
        """The cap limits LLM cost, and now degrades rather than failing.

        It used to 409 the whole request. Since scoping became deterministic-first
        that would lock an auditor out of *mechanical* scoping — which costs
        nothing and calls nothing — for the rest of the hour, on the basis of an
        LLM budget. The request now succeeds with the deterministic result and
        reports that the advisory pass did not run.
        """
        ...
        assert response.status_code == 200
        # Honest about the degradation rather than silently returning less.
        assert response.json()["manual_scoping_required"] is True
...
class TestReRunIdempotency:
    def test_rerun_replaces_unconfirmed_suggestions(
        ...
        assert "3.3.1" in [r["control_id"] for r in body["proposed_requirements"]]

        rows = db.scalars(select(ScopedControl).where(ScopedControl.audit_id == audit.id)).all()
        ai_rows = {r.control.control_id for r in rows if r.source is ScopeSource.ai_suggested}
        # The prior run's advisory rows are gone; only the newest remain.
        assert ai_rows == {"3.3.1"}

        # Deterministic exclusions coexist with them and are never replaced by a
        # re-run — they are a mechanical determination, not a suggestion.
        deterministic = {
            r.control.control_id for r in rows if r.source is ScopeSource.deterministic
        }
        assert all(
            r.applicability_status is ApplicabilityStatus.NOT_APPLICABLE
            for r in rows
            if r.source is ScopeSource.deterministic
        )
        # This audit is a merchant, so every service-provider-only control is
        # excluded by rule rather than left for a human to notice.
        assert "12.9.1" in deterministic
    ...
    def test_rerun_never_touches_confirmed_rows(
        ...
        set_llm_client(FakeLLM(suggestion_payload("1.2.1", "1.2.2")))
        first = api_client.post(f"/api/audits/{audit.id}/scope-suggestion").json()
        keep = next(r for r in first["proposed_requirements"] if r["control_id"] == "1.2.1")
        ...
        rows = db.scalars(select(ScopedControl).where(ScopedControl.audit_id == audit.id)).all()
        by_clause = {r.control.control_id: r for r in rows}
    ...
    def test_rerun_does_not_duplicate_a_manually_added_clause(
        ...
        listed = api_client.get(f"/api/audits/{audit.id}/scoped-requirements").json()
        # The point of this test is that a re-run does not duplicate the clause,
        # and does not relabel a human's row as the model's. Deterministic
        # exclusions also live in this list now, so assert on 1.2.1 specifically
        # rather than on the whole list.
        rows_121 = [r for r in listed if r["control_id"] == "1.2.1"]
        assert len(rows_121) == 1
        assert rows_121[0]["source"] == "manual"


class TestConfirmation:
    def test_confirming_sets_the_flag_and_advances_the_audit(
        self,
        api_client: TestClient,
        db: DBSession,
        make_user: Any,
        ...
        audit = make_audit(auditor)
        login(api_client, auditor)
        proposed = api_client.post(f"/api/audits/{audit.id}/scope-suggestion").json()[
    ...
    def test_nothing_is_confirmed_without_an_explicit_human_action(
        ...
        never mark a ScopedControl confirmed without a human action."""
        ...
        rows = db.scalars(select(ScopedControl).where(ScopedControl.audit_id == audit.id)).all()
        assert rows
        # The invariant: nothing reaches `confirmed` without a human, whatever
        # produced the row. It now has to hold for deterministically-derived rows
        # too — a mechanical determination is still not a human decision.
        assert all(r.confirmed is False for r in rows)
        assert {r.source for r in rows} <= {
            ScopeSource.ai_suggested,
